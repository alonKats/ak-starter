...
for i in `grep -rl ak-starter * --exclude-dir=node_modules 2> /dev/null`; do LC_ALL=C sed -i '' -e "s;ak-starter;${THEME_NAME};" $i $i; done
for i in `grep -rl Air-light * --exclude-dir=node_modules 2> /dev/null`; do LC_ALL=C sed -i '' -e "s;Air-light;${THEME_NAME};" $i $i; done
for i in `grep -rl air * --exclude-dir=node_modules 2> /dev/null`; do LC_ALL=C sed -i '' -e "s;ak-starter;${THEME_NAME};" $i $i; done
